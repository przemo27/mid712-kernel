...
static int lowmem_shrink(int nr_to_scan, gfp_t gfp_mask)
{
	...
	if(nr_to_scan > 0)
		lowmem_print(3, "lowmem_shrink:: nr_to_scan=%d, gfp_mask=%x, other_free=%d, min_adj=%d\n", nr_to_scan, gfp_mask, other_free, min_adj);
		...
		if(p->oomkilladj >= 0 && p->mm) {
			...
			if(nr_to_scan > 0 && tasksize > 0 && p->oomkilladj >= min_adj) {
				if(selected == NULL || p->oomkilladj > selected->oomkilladj ||(p->oomkilladj == selected->oomkilladj && tasksize > selected_tasksize)) {
					...
					lowmem_print(2, "select p->pid=%d (p->comm=%s), p->oomkilladj=%d, tasksize=%d, to kill\n", p->pid, p->comm, p->oomkilladj, tasksize);
				}
			}
			rem += tasksize;
		}
	...
	if(selected != NULL) {
		lowmem_print(1, "send sigkill to:: selected->pid=%d (selected->comm=%s), selected->oomkilladj=%d, selected_tasksize=%d\n",selected->pid, selected->comm,selected->oomkilladj, selected_tasksize);
		force_sig(SIGKILL, selected);
		rem -= selected_tasksize;
	}
	lowmem_print(4, "lowmem_shrink:: nr_to_scan=%d, gfp_mask=%x, return  rem=%d\n", nr_to_scan, gfp_mask, rem);
