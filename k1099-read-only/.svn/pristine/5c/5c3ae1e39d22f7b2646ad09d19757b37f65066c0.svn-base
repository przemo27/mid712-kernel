...
volatile int    pm_onelevel_status = 0;  // 1:enter 1 level sleep
extern int g_wakeup_key_enable;
extern struct timer_list g_wakeup_key_timer;
extern int g_enable_sleep;
...
extern unsigned int usbchargestatus;	/*usb charge status*/
#endif
...
/* 20091112,HSL@RK , full lock system */
void android_lock_partial_suspend(android_suspend_lock_t *lock)
{
	unsigned long irqflags;
	spin_lock_irqsave(&g_list_lock, irqflags);
#ifdef CONFIG_ANDROID_POWER_STAT
	...
	lock->expires = INT_MAX;
	lock->flags &= ~ANDROID_SUSPEND_LOCK_AUTO_EXPIRE;
	lock->flags |= ANDROID_SUSPEND_LOCK_AWAKE;
	list_del(&lock->link);
	list_add(&lock->link, &g_active_full_wake_locks);
	g_current_event_num++;
	spin_unlock_irqrestore(&g_list_lock, irqflags);
...
static void android_power_request_sleep(void)
{
	...
	list_for_each_entry_safe(lock, next_lock, &g_active_full_wake_locks, link) {
		...
		android_unlock_suspend_stat_locked(lock);
#endif
                                if( (ANDROID_SUSPEND_LOCK_AWAKE&lock->flags) ) {
                                        printk("%s::lock %s state at full wake\n" , __func__ , lock->name );
                                        continue;
                                }
		list_del(&lock->link);
		list_add(&lock->link, &g_inactive_locks);
...
static void android_power_suspend(struct work_struct *work)
{
	...
	while(g_user_suspend_state != USER_AWAKE) {
		while(g_user_suspend_state == USER_NOTIFICATION) {
			...
			if(wait)
				wait_event_interruptible_timeout(g_wait_queue, get_wait_timeout(0, USER_NOTIFICATION, &g_active_full_wake_locks) != wait, wait);
                                ...
                                #if 1
		while( 1 ) {
		        wait = get_wait_timeout(print_locks, USER_SLEEP, &g_active_full_wake_locks);
		        if( wait < 0 )
		                break;
		        wait_event_interruptible(g_wait_queue, get_wait_timeout(0, USER_SLEEP, &g_active_full_wake_locks) != wait);
		}
                                #endif
		...
		mutex_lock(&g_early_suspend_lock);
		//printk("android_power_suspend: begin call early suspend handlers\n");
		list_for_each_entry(pos, &g_early_suspend_handlers, link) {
			if(pos->suspend != NULL)
				pos->suspend(pos);
		}
		//printk("android_power_suspend: after call early suspend handlers\n");
		arm_clk_mHz = rockchip_clk_get_arm()/1000000;
#ifdef CONFIG_MACH_LANMO_W7
		if((GPIOGetPinLevel(GPIOPortB_Pin1) && get_msc_connect_flag()))/*not usb data transform mode ,can change freqency*/
#endif
                                printk("android_power_suspend:Current arm_clk==%dMHz\n",arm_clk_mHz);
        	                rockchip_clk_set_arm(300);
                                pm_onelevel_status = 1;  
		//sys_sync();
		while(g_user_suspend_state == USER_SLEEP) {
			...
			//sys_sync();
			if (android_power_debug_mask & ANDROID_POWER_DEBUG_SUSPEND)
				printk(KERN_INFO "android_power_suspend: enter pm suspend\n");
		    g_enable_sleep = 0;
		...
		//printk("android_power_suspend: set arm clk to 600 M\n");
		curren_clk = rockchip_clk_get_arm()/1000000;
		printk("android_power_suspend: after 2 level sleep wake up current_clk==%dmHz  arm_clk_mHz==%dmHz\n",curren_clk,arm_clk_mHz);
                rockchip_clk_set_arm( arm_clk_mHz );
		list_for_each_entry_reverse(pos, &g_early_suspend_handlers, link) {
			if(pos->resume != NULL)
				pos->resume(pos);
		}
		pm_onelevel_status = 0;
        g_enable_sleep = 1;
		//printk("android_power_suspend: call late resume handlers\n");
		mutex_unlock(&g_early_suspend_lock);
	}
...
/*
 * 20100309,HSL@RK,fun for print all current locks.open when need.
 */
#if 0 
static void android_lock_print(char* list_name , struct list_head *list_head)
{
	unsigned long irqflags;
	android_suspend_lock_t *lock, *next;
	int             i = 0;
...
int android_power_print_locks( int lock_masks )
{
        struct list_head * locks[] = {
                &g_inactive_locks,
                &g_active_idle_wake_locks,
                &g_active_partial_wake_locks,
                &g_active_full_wake_locks,
                NULL,
        };
        char * names[] = {
                "g_inactive_locks","g_active_idle_wake_locks",
                "g_active_partial_wake_locks(lock for deep suspend)",
                "g_active_full_wake_locks(lock for early suspend)",NULL
        };
        struct list_head * *now =  &locks[0];
        int lock_num = 0;
        while( *now ) {
                if( (lock_masks&1) )
                        android_lock_print( names[lock_num] , *now );
                now++;
                lock_masks >>= 1;
                lock_num++;
        }
...
static ssize_t request_state_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
	if(n >= strlen("standby") &&
	   strncmp(buf, "standby", strlen("standby")) == 0) {
		android_power_request_sleep(); /* call at PowerManagerSer */
		return n;
	}
	if(n >= strlen("wake") &&
	   strncmp(buf, "wake", strlen("wake")) == 0) {
