...
#define MAC_INT_PORT GPIOPortE_Pin1
#else
#define MAC_INT_PORT GPIOPortE_Pin2
#endif
enum {
	RXFILTER_NORMAL,
	RXFILTER_MULTI,
...
spi_read_buf(struct enc28j60_net *priv, int len, u8 *data)
{
	...
	if (ret == 0) {
	memcpy(data, &rx_buf[0], len);  //SPI_OPLEN], len);
		ret = msg.status;
	}
...
///__raw_writel(__raw_readl(GPIO1_BASE_ADDR_VA + 0x4c) | 0x40,(GPIO1_BASE_ADDR_VA + 0x4c)); 
	/*
...
int enc28j60_suspend(suspend_state_t state)
{
	printk("%s [%d]\n",__FUNCTION__,__LINE__);
	enc28j60_net_close(g_ndev);
	return 0;
}

void enc28j60_resume(void)
{
	printk("%s [%d]\n",__FUNCTION__,__LINE__);
	enc28j60_net_open(g_ndev);
}
...
static int __devinit enc28j60_probe(struct spi_device *spi)
{
	...
	enc28j60_set_hw_macaddr(dev);
//__raw_writel(__raw_readl(GPIO1_BASE_ADDR_VA + 0x30) | 0x40,(GPIO1_BASE_ADDR_VA + 0x30));
	//ret = request_irq(7, enc28j60_irq, IRQF_TRIGGER_FALLING,
			 // DRV_NAME, priv);
	GPIOPullUpDown(MAC_INT_PORT,GPIOPullUp);		 
	ret = request_gpio_irq(MAC_INT_PORT,enc28j60_irq,GPIOEdgelFalling,priv);		
	if (ret < 0) {
		if (netif_msg_probe(priv))
