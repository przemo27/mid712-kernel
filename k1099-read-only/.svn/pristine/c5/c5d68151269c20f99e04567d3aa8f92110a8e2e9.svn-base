...
static int dm_write_shared_word(struct usbnet *dev, int phy, u8 reg, __le16 value)
{
	...
	dm_write_reg(dev, DM_SHARED_ADDR, phy ? (reg | 0x40) : reg);
	dm_write_reg(dev, DM_SHARED_CTRL, phy ? 0x0a : 0x02);
...
dm9620_set_wol(struct net_device *net, struct ethtool_wolinfo *wolinfo)
{
	...
	dm_write_reg(dev, 0x0, 0x48);  // enable WAKEEN 
...
static const struct net_device_ops vm_netdev_ops= { // new kernel 2.6.31  (20091217JJ)
            ...
            .ndo_open               = usbnet_open,  //423
            .ndo_stop               = usbnet_stop,  //424
            .ndo_start_xmit         = usbnet_start_xmit, //425
            .ndo_tx_timeout         = usbnet_tx_timeout, //426
            .ndo_change_mtu         = usbnet_change_mtu, //427
            .ndo_validate_addr      = eth_validate_addr, //428
...
//430       .ndo_set_multicast_list = dm9601_set_multicast,
            .ndo_set_mac_address    = dm9620_set_mac_address,  //431
};
#endif
...
static int dm9620_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	/*bill, 2008/11/26*/
	//printk("20081126 bill debug\n");
	//dm_write_reg(dev, 0x2e, 0x20);
	//dm_write_reg(dev, 0xff, 0x80);
	//printk("20090812 bill debug\n");
	//dm_write_reg(dev, 0x0f, 0x1 << 5);
	//dm_write_reg(dev, 0x0, 0x1 << 6 );

	/* Add V1.1, Enable auto link while plug in RJ45, Hank July 20, 2009*/
	dm_write_reg(dev, 0xf4, 0x20); 
	/* read MAC */
	...
	printk("Fixme: work around for 9620 mode\n");
	printk("Add tx_fixup() debug...\n");
	dm_write_reg(dev, 0x16, 0);
	dm_read_reg(dev, 0x16, &temp); 
	ret = dm_read_reg(dev, DM_SMIREG, &temp);

	priv->flag_fail_count= 0;
	if (ret<0) {
		printk(KERN_ERR "Error read SMI register\n");
	}
	else priv->mode_9620 = temp & DM_MODE9620;

	printk(KERN_WARNING "9620 Mode = %d\n", priv->mode_9620);
	
	/* power up phy */
	dm_write_reg(dev, DM_GPR_CTRL, 1);
	dm_write_reg(dev, DM_GPR_DATA, 0);
	/* Init tx/rx checksum */
	dm_write_reg(dev, 0x31, 7);
	dm_write_reg(dev, 0x32, 3);
	...
	dm9620_mdio_write(dev->net, dev->mii.phy_id, 20, 0x800);
	printk("dm962++ write phy[20]= 0x800\n");
	mdio_val = dm9620_mdio_read(dev->net, dev->mii.phy_id, 20);
	printk("dm962++ read  phy[20]= %x\n",mdio_val );
...
/*
	...
	//read phy 20 to temp//
	//printk("phy[20]=%d\n",dm9620_mdio_read(netdev, phy_id, 20)); 
	temp=dm9620_mdio_read(dev->net, 1, 20);
	printk("dm9620 in tx read  phy[20]= %d\n",temp );
	}
          */
	/* format:
