...
/* 20091112,HSL@RK */
static int ashmem_check_file( struct file *file ) 
{
        if( file->private_data <= (void*)0xc0400000 ) {
                printk("%s warning:: file %s error ,f=0x%p, f_op=0x%p,f_nrwrite=0x%x,private_data=0x%p\n" , __func__ ,  file->f_path.dentry->d_name.name , 
                        file , file->f_op, file->f_nr_writes , file->private_data );
                //return 0;       /* for found BUG !!*/
                
                //to be fix, if private_data equal 0, we return err, but we don't know why private_data changed to 0
                return -EIO;
        }
        return 0;
}
#define ASHMEM_CHECK_FILE( file )       do{ if( ashmem_check_file(file ) ) return -EIO; }while( 0 )
