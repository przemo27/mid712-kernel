...
static int __init init(void)
{
	...
	if (!dev){
		printk(KERN_INFO "android init kzalloc fail \n");
		return -ENOMEM;
	}
	...
	if (ret){
		printk(KERN_INFO "android init fail: platform_driver_register error \n");		
		return ret;
	}
	ret = misc_register(&adb_enable_device);
	if (ret) {
		printk(KERN_INFO "android init fail: misc_register error \n");		
		platform_driver_unregister(&android_platform_driver);
		return ret;
	}
	ret = usb_composite_register(&android_usb_driver);
	if (ret) {
		printk(KERN_INFO "android init fail: usb_composite_register error \n");		
