...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if(alarm_type >= ANDROID_ALARM_TYPE_COUNT)
		return -EINVAL;
	if(cmd == ANDROID_ALARM_SET_SPEAKER)
	{
		
			if(copy_from_user(&speaker_status, (void __user *)arg, sizeof(unsigned int))) {
				printk("android set speaker error! \n");
				return -1;
			}
			#if 0
			if(speaker_status == 1) /*enable speaker*/
			{
				rockchip_mux_api_set(GPIOE_SPI1_SEL_NAME, IOMUXA_GPIO1_A1237);/*speaker enable pin PF7*/
...
from_old_alarm_set:
		...
		case ANDROID_ALARM_SET_RTC:
			...
			printk("set rtc  - rtc %02d:%02d:%02d %02d/%02d/%04d week=%02d\n",
				rtc_new_rtc_time.tm_hour, rtc_new_rtc_time.tm_min,
				rtc_new_rtc_time.tm_sec, rtc_new_rtc_time.tm_mon + 1,
				rtc_new_rtc_time.tm_mday, rtc_new_rtc_time.tm_year + 1900,rtc_new_rtc_time.tm_wday);
			rv = rtc_set_time(alarm_rtc_dev, &rtc_new_rtc_time);
			rtc_read_time(alarm_rtc_dev, &rtc_current_rtc_time);
			printk("Read rtc  - rtc %02d:%02d:%02d %02d/%02d/%04d week=%02d\n",
				rtc_current_rtc_time.tm_hour, rtc_current_rtc_time.tm_min,
				rtc_current_rtc_time.tm_sec, rtc_current_rtc_time.tm_mon + 1,
				rtc_current_rtc_time.tm_mday, rtc_current_rtc_time.tm_year + 1900,rtc_current_rtc_time.tm_wday);
			spin_lock_irqsave(&alarm_slock, flags);
...
int alarm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	struct rtc_time     rtc_current_rtc_time;
	struct rtc_time 	rtc_alarm_rtc_time;
	...
	if(alarm_enabled & (ANDROID_ALARM_RTC_WAKEUP_MASK | ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP_MASK)) {
		...
		rtc_read_time(alarm_rtc_dev, &rtc_current_rtc_time);
		rtc_set_time(alarm_rtc_dev, &rtc_current_rtc_time);
		...
		rtc_time_to_tm(rtc_alarm_time, &rtc_alarm_rtc_time);
		rtc_alarm.time.tm_sec = rtc_alarm_rtc_time.tm_sec;
		printk("rtc_alarm.time.tm_sec ==%d \n",rtc_alarm.time.tm_sec);
		if(rtc_alarm.time.tm_sec > 0)
			rtc_alarm.time.tm_min = rtc_alarm_rtc_time.tm_min + 1;
		else
			rtc_alarm.time.tm_min = rtc_alarm_rtc_time.tm_min;
		...
		rtc_alarm.enabled = 1;
		printk("set alarm  - rtc %02d:%02d:%02d %02d/%02d/%04d week=%02d\n",
		...
		printk("****************%s-->rtc alarm set at %ld, now %ld, rtc delta %ld.%09ld---->PE2=%d\n",
			__FUNCTION__,rtc_alarm_time, rtc_current_time,
			rtc_delta.tv_sec, rtc_delta.tv_nsec,GPIOGetPinLevel(GPIOPortE_Pin2));
		if(rtc_current_time + 1 >= rtc_alarm_time) {
			...
			rtc_alarm.enabled = 0;
			rtc_set_alarm(alarm_rtc_dev, &rtc_alarm);
			printk("**************--->%s-->alarm about to go off--->PE2==%d\n",__FUNCTION__,GPIOGetPinLevel(GPIOPortE_Pin2));
			spin_lock_irqsave(&alarm_slock, flags);
...
int alarm_resume(struct platform_device *pdev)
{
	...
	if(alarm_enabled & (ANDROID_ALARM_RTC_WAKEUP_MASK | ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP_MASK)) {
		...
		printk("************%s-->alarm_resume(%p)  PE2==%d\n", __FUNCTION__,pdev,GPIOGetPinLevel(GPIOPortE_Pin2));
		alarm_start_hrtimer(ANDROID_ALARM_RTC_WAKEUP);
		alarm_start_hrtimer(ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP);
	}
...
static int __init alarm_init(void)
{
	...
	for(i = 0; i < ANDROID_ALARM_TYPE_COUNT /*ANDROID_ALARM_SYSTEMTIME*/; i++) {
