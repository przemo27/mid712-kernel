...
#define BATT_VOLTAGE_MIN	3350 //3500
#define AD_SAMPLE_TIMES	6

#define AD_NO_BATT_VALE       200
#define AD_NO_DC_VALE         250
...
static int batt_step_table[56]={
    3420,3450,3470,3500,3540,3545,3555,3560,3580,3600,3615,3630,3640,3650,3660,3670,3680,3690,
...
static int batt_no_current_step_table[56]={
	3500,3540,3600,3650,3680,3690,3700,3715,3725,3740,3745,3755,3765,3770,3778,3784,3790,3800,3810,
...
static int batt_disp_table[56]={
    1,3,5,8,10,12,14,15,18,20,23,26,28,30,33,37,40,43,47,
...
static u16 g_adc_dc_value = 0;
static u16 g_adccharge = 0;
extern	u16 get_rock_adc0(void);	/*battery capacity*/
extern	u16 get_rock_adc1(void);	/*battery charge end status*/
extern	u16 get_rock_adc2(void);	/*battery charge status*/
...
static int rockchip_get_battery_status(void)
{
    ...
    g_adc_dc_value = (g_adc_dc_value + get_rock_adc1())/2;
    g_adccharge = (g_adccharge + get_rock_adc2())/2;
...
#if 1    
    if(g_adccharge > AD_NO_DC_VALE)	/*enter ac supply status*/
	{
		bat_status = POWER_SUPPLY_STATUS_CHARGING;
		if(g_adc_dc_value>800){	/*the battery full*/
			g_charge_full_cnt++;
			if(g_charge_full_cnt > 30){
			    bat_status =POWER_SUPPLY_STATUS_FULL ;
			    g_charge_full_cnt = 30;
			    bat_capacity = 100;
		        return 1;
		    }
		}else{
	...
	bat_vol_no_power_cnt = 0;
	if(g_adc_dc_value>AD_NO_DC_VALE){
		for(i=0; i<56; i++)		
 ...
 static enum hrtimer_restart rk28_battery_dostimer(struct hrtimer *handle)
{
	...
	{
		if((g_adccharge>AD_NO_DC_VALE)&&(bat_capacity<14))
			bat_capacity = 16;	
...
static int rockchip_battery_probe(struct platform_device *pdev)
{
	...
	#if USB_SUPPLY_POWER
	data ->usb	  = rockchip_power_supplies[2];
	#endif
	data ->ac        = rockchip_power_supplies[1]; ///2];
	...
	DBG("---->battery current voltage = %d\n",bat_voltage);
	DBG("---->battery capacity = %d\n",bat_capacity);	
	if((bat_capacity <14 )&&(g_adccharge>AD_NO_DC_VALE))
		bat_capacity = 16;
	if((bat_capacity == 0 )&&(g_adccharge<=AD_NO_DC_VALE))
		 rk28_power_off();
