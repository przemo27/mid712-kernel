...
#else
const static __u32*
format_by_fourcc(struct soc_camera_host *ici, unsigned int fourcc)
{
	unsigned int i;

	for (i = 0; i < ici->num_formats; i++)
		if (ici->fourcc[i] == fourcc)
			return ici->fourcc + i;
	return NULL;
}
#endif
				  ...
				  struct v4l2_format *f)
{
	...
	const struct soc_camera_data_format *sen_fmt = icd->formats;
	const __u32 *fmt;
	int ret;
	...
	fmt = format_by_fourcc(ici, f->fmt.pix.pixelformat);
	if (!fmt) {
	...
	field = V4L2_FIELD_NONE;//f->fmt.pix.field;
#if 0
	...
	f->fmt.pix.bytesperline =
		(f->fmt.pix.width * sen_fmt->depth) >> 3;
	f->fmt.pix.sizeimage =
			      ...
			      struct v4l2_requestbuffers *p)
{
    ...
    icf->vb_vidq.buf_purpose = p->reserved[0];//nzy add
...
static int soc_camera_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    printk("\n%s..%s..%d    ******** nzy *********%d %d\n",__FUNCTION__,__FILE__,__LINE__, icd->width, icd->height);
				...
				struct v4l2_format *f)
{
	...
	const static struct soc_camera_data_format *sen_fmt;
	const __u32 *data_fmt;
	...
	data_fmt = format_by_fourcc(ici, f->fmt.pix.pixelformat);
	if (!data_fmt)
		return -EINVAL;

	/* buswidth may be further adjusted by the ici */
	icd->buswidth = sen_fmt->depth;
