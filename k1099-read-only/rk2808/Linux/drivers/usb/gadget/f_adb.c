...
/* 20100206,HSL@RK,for rkdebug. */
#include<asm/arch/rk28_msc_ext.h>
extern int     adb_support ;

#define BULK_BUFFER_SIZE           4096

/* number of rx and tx requests to allocate */
#define RX_REQ_MAX (4+1)
#define TX_REQ_MAX 4
...
static void adb_complete_out(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->status != 0) {
		...
		req_put(dev, &dev->rx_idle, req);
	                ...
	                RKUSB_OUT(adb,&dev->rk_dev,ep,req);
...
#if 1
static int adb_rkinit( struct adb_dev	*dev )
{
        struct usb_request *req;
        int     ret = -EIO;
        while ((req = req_get(dev, &dev->rx_idle))) {
                //printk("%s::get cmd for krusb,req=0x%p\n" ,__func__, req);
        	req->length = BULK_BUFFER_SIZE;
        	ret = usb_ep_queue(dev->ep_out, req, GFP_ATOMIC);
		...
		unsigned intf, unsigned alt)
{
	...
	wake_up(&dev->read_wq);
	RKUSB_OPEN(&dev->rk_dev,dev->ep_in,dev->ep_out,0);	
	adb_rkinit( dev );
	return 0;
}
...
int __init adb_function_add(struct usb_configuration *c)
{
	...
	dev->function.forbid = 0; /* default:open */
	dev->function.bind = adb_function_bind;
	dev->function.unbind = adb_function_unbind;
	dev->function.set_alt = adb_function_set_alt;
	dev->function.disable = adb_function_disable;
             dev->function.idProduct = 0x0C02;
             dev->function.idVendor = 0x0BB4;
	...
	if (ret)
		goto err2;
                ...
                adb_support = 1;
                adb_function_enable( 1 ); /* 20100127,HSL@RK,for test */
	return 0;
...
/*
 * 20100312,HSL@RK,use for get whether adb enabled now.
 * call when start msc cmd check timer.
 */
int adb_enabled( void )
{
        struct adb_dev *dev = _adb_dev;
        if(dev && !dev->function.forbid )
                return 1;
        return 0;
}
