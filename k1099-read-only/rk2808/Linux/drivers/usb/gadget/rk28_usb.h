...
/******************************************************************/
/* kernel support version .
 * 10200330,HSL@RK,ADD VERSION CTRL.
*/
#define RKUSB_KVERSION          "01.0.0"         
#define RKUSB_TVERSION          "01.0.0"         /* pc tool version */
...
#define   K_FW_FUNCALL				(RKUSB_CMD_BASE+7)  //XXX: call functions in task env .
...
/* CMD K_FW_USBCMD */
#define FUNC_UCMD_DEVREADY                0 /* device ready or not,and reset dev to cmd phase */
#define FUNC_UCMD_DISCONNECTMSC     1
#define FUNC_UCMD_SYSTEMINFO            2 /* get system info,string */
#define FUNC_UCMD_DEBUGINFO              3 /* debug infor,for pc tool ctrl . */
#define FUNC_UCMD_OPENADB                  4 /* MSC NOT stable for large data transfer,so use adb instead. */
...
#define FUNC_GSYM_PROFILE               3 /* get profile data */
...
/* 20100225,HSL@RK,if panic,adb can not get log. */
#define LOG_NUM                                 2 
...
struct rkusb_dev {
            ...
            int                   buf_length;
            char                *req_in_buf;
            int                   req_buf_length; /* in req buffer length(max packet size) */
            int                   buf_filled;
            int                   buf_nfill;
            int                   fsg;
};
...
#define __rkusb_clear_cb( dev )         //do{ dev->fsg_dev->usb_in = NULL;}while( 0 )
#define __rkusb_set_epin_cb( dev , cb )  //do{ dev->fsg_dev->usb_in = cb; }while(0)
#define rkusb_wakeup_thread( dev )              do{schedule_work(&dev->theadwork);}while(0)
#define rkusb_sleep_thread( dev )                   
...
/* 20100323,HSL@RK,for compatable for future,ALL struct may change will have the size as first element
 * , use the define below to get next ptr.and the size use the value read from system.
*/
#define NEXT_OBJ( t , p )               ((t*)(((char*)p)+  ((t*)p)->size ))
#define NEXT_OBJP( p )               ({typeof(p) __p; __p=((((char*)p)+  (p)->size ));__p;}) // only use at linux.
#endif          /* define __KERNEL__  */

typedef struct __rkdebug_info{
        int             size;           /* 20100324,HSL@RK,struct size */
        
        int             tr_length;      /* max buffer size */
        int             save_length;      /* save tmp buffer */
        unsigned long save_buf;      /* save tmp buffer ptr */
        int             dbg_level;      /* 1.only msc ext,2.may read,3.may write */
        int             adb_support;       /* */
        char           cur_device[8];       /* current use msc or adb */
        char           cur_version[8];      /* 20100324,HSL@RK,add version control */
        unsigned char cmds[16];  /* max cmds 16*/

} RKDEBUG_INFO;
extern RKDEBUG_INFO    g_dgb_info;
...
typedef struct __task_info{
        ...
        void *stack;           
        unsigned int flags;	/* 20100324,FLAG is import,need to add */
        int prio, static_prio, normal_prio;
        void *sched_class;
        u64			sum_exec_runtime; /* ns-->to us */
        void *mm, *active_mm; /* active mm for task cmdline name. */
        pid_t pid;
        ...
        /* for schedule info */
        unsigned int policy;    /* 20100326,HSL@RK,connect with sched_class,may get 
                                        per_cpu__runqueues for current rq,rq->curr to get current task */
} TASK_INFO;
...
typedef struct __task_vam_info{
        ...
        unsigned long end;
        unsigned long flags;
        unsigned long pgoff;    /* 20100226,HSL@RK,for file offset!*/
        void               *file;
...
#define BKP_FLAG_STOP_WHILE                      (1<<1) /* WAIT OR WHILE */
#define BKP_FLAG_CHECK                                 (1<<2)  /* have check */
#define BKP_FLAG_NOSTACK                                 (1<<3)  /* 20100324,HSL@RK,not print stask */
...
#define BKP_FLAG_SOURE_REG( flag )          ((flag>>24)&0xf) /* reg index r0-r13 */
#define BKP_CMP_MAX_REG                        0xf /* the flag of invalid reg index  */
#define BKP_FLAG_SOURE_SIZE( flag )       (((flag>>28)&0x3) ) /* 0:4BYTE,1:2,2:1,3:reserve */
#define BKP_FLAG_CMP_USER                       (1<<30)         /* CMP by user define function */
#define BKP_FLAG_CMP_PID                         (1<<31)        /* cmp task pid */
...
typedef struct __profile_info{
        int                  size;
        unsigned long   npc;    /* current len */
        unsigned long   buf;    /* data buffer */
        u64	                now;   /* device timer */
} PROFILE_INFO;

/*
 *  profile funtions entry.
 *
*/
...
typedef struct __profile_cpu_ks {
        cputime64_t utime;
        cputime64_t ntime;
        cputime64_t stime;
        cputime64_t sirqtime;
        cputime64_t irqtime;
        cputime64_t itime;
        cputime64_t iowtime;
        cputime64_t steal;
...
/* 20100315,HSL@RK,define for insert sort. */
struct sort_oop{
	int 	(*en_cmp)( void *a , void *b );
	void (*en_print)( void *a , int sorted );
	void (*en_swap)( void *a , void *b );
	void* (*en_next)( void ***p,void** end); /* keep *p > end as terminate .*/
};
...
/* sort array at descrease(desc=1) or increase mode.if op == NULL, treat as int.
 *  return : the sorted single list , head: the malloc single list for free outside this function.
 *  if head == NULL,free at the insert_sort,and return NULL .
 */
struct single_list * insert_sort( void** arr , void** end,int size, int desc, struct sort_oop * op ,
	 struct single_list ** head );

typedef struct __cmd_item {
        char * cmd;
        int (*doing)(int argc, UINT argv[]);
} CMD_ITEM;     /* for sub sub function */
#define LAST_ITEM       {"",NULL}
int doing_cmd( int argc , UINT argv[] , CMD_ITEM* table );

/*size of all parts. */
typedef struct __task_crash{
        int     size;
        int     task;   /* task info */
        int     mm;     /* mm info */
        int     vma;   /* all vmas */
        int     stask;  /* crash stack .*/
} TASK_CRASH;
