 ...
 * WLAN Device
 */
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/releases/olca2.2/host/os/linux/ar6000_drv.c#21 $";
...
struct wake_lock ar6k_connect_wake_lock;
struct wake_lock ar6k_init_wake_lock;
static struct net_device *pending_devs[MAX_AR6000];

#define FWPATH "/etc/firmware/"
char *tgt_fw = FWPATH"athwlan.bin.z77";
char *tgt_patch = FWPATH"data.patch.hw2_0.bin";
char *tcmd_fw = FWPATH"athtcmd_ram.bin";
char *art_fw = FWPATH"device.bin";
char *eeprom_bin = FWPATH"eeprom.bin";
char *eeprom_data = FWPATH"eeprom.data";


#ifdef REGION_CODE_FILE_USED
char *reg_file = FWPATH"reg_code";
#else
char *reg_file = NULL;
#endif

#ifdef FAKE_EEPROM_USED
//char *eeprom_data = "/system/wifi/calData_ar6102_15dBm.bin";
//char *eeprom_file = "/system/wifi/fakeBoardData_AR6002.bin";
char *eeprom_file = FWPATH"calData_ar6102_15dBm.bin";
char *softmac_file = FWPATH"softmac";
#else
...
/* ATHENV */
	...
	if (wifi_mmc_host != NULL)  /* There is a Host controller. */
	{
		if (wifi_mmc_host->bus_ops == NULL) /* No mmc/card is being attached. */
		{
			if (wifi_turn_on_card() != 0)
				return -1;

			if (wifi_power_up_wifi() != 0)
				return -1;
				
			mmc_detect_change(wifi_mmc_host, 10);
			{
				int timeout;
	...
	{
		printk("No SDMMC host for wifi is active.\n");
		return -1;
	}
#endif
...
/* ATHENV */
#ifdef ANDROID_ENV
    wake_lock_init(&ar6k_connect_wake_lock, WAKE_LOCK_SUSPEND, "ath_connect");
...
/* ATHENV */
#ifdef ANDROID_ENV
    wake_lock_destroy(&ar6k_connect_wake_lock);
...
/* ATHENV */
...
#ifdef FAKE_EEPROM_USED  /* eeprom file -> Host -> FW */
	...
	printk("After eeprom_ar6000_transfer: mac address: %x:%x:%x:%x:%x:%x\n", 
	ar->arNetDev->dev_addr[0],
	ar->arNetDev->dev_addr[1],
	ar->arNetDev->dev_addr[2],
	ar->arNetDev->dev_addr[3],
	ar->arNetDev->dev_addr[4],
	ar->arNetDev->dev_addr[5]);
...
#define AC_NOT_MAPPED   99
    ...
    if ((ac == AC_NOT_MAPPED) && !checkAdHocPsMapping) {
            /* cleanup and exit */
        A_NETBUF_FREE(skb);
		printk("1111: ar6000_data_tx: dropped.\n");
    ...
    if (cookie != NULL) {
        ...
        A_NETBUF_FREE(skb);
        AR6000_STAT_INC(ar, tx_dropped);
		printk("2222: tx_aborted_errors: dropped.\n");
        AR6000_STAT_INC(ar, tx_aborted_errors);
    }
...
static HTC_SEND_FULL_ACTION ar6000_tx_queue_full(void *Context, HTC_PACKET *pPacket)
{
    ...
    do {
        ...
        if (Endpoint == ar->arControlEp) {
            ...
            printk("WMI Control Endpoint is FULL!!! \n");
        ...
        if (ar->arAcStreamPriMap[arEndpoint2Ac(ar,Endpoint)] < ar->arHiAcStreamActivePri) {
			printk("this stream's priority is less than the highest active priority.\n");
...
#ifdef ANDROID_ENV
    /* prevent suspend */
    //wake_lock(&ar6k_connect_wake_lock);
    //printk("WLAN Connect: avoid to suspend.\n");
#endif
/* ATHENV */
...
/* ATHENV */
    ...
    /* able to suspend now */
    //wake_unlock(&ar6k_connect_wake_lock);
    //printk("WLAN Disconnect: able to suspend.\n");
#endif
/* ATHENV */
}
